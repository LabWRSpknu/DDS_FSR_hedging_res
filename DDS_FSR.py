...
#======================== Brief Description ====================================
# This script includes main DDS-FSR algorithm.
# The DDS-FSR is modified DDS-Py.
# The DDS-Py was coded on Aug 2015 by Thouheed Abdul Gaffoor.
# This script was coded to derive hedigng rule curves by Y. Jin
#===============================================================================

def DDS_serial(objfunc_name, exe_name, modeldir, to_max, DV, sinitial, its, maxiter,
               Total_Copacity, lower_Capacity, Initial_Stor, Inflow_TS, Target_12TS, Ration_Ratio_48TS):
    # ==========================================================================
    # Definitions
    # ==========================================================================
    ...
    for i in range(its):
        if its > 1:
            if Discrete_flag.all() == 0:  # handling continuous variables
                ...
                for j in range(0, num_dec):
                    ...
                    stest[j] = np.random.randint([DV['S_min'][j], DV['S_max'][j]], size=1)
        ...
        Jtest = to_max * util.get_objfunc(stest, modeldir, objfunc_name, exe_name, 0,
                                          Total_Copacity, lower_Capacity, Initial_Stor, Inflow_TS,
                                          Target_12TS, Ration_Ratio_48TS)
    ...
    for i in range(ileft):
        ...
        for j in range(num_dec):
            ...
            if j < 12:
                stest_upper = DV['S_max'][j]
                stest_lower = stest[j + 12]
            elif j >= 36:
                stest_upper = stest[j - 12]
                stest_lower = DV['S_min'][j]
            else:
                stest_upper = stest[j - 12]
                stest_lower = stest[j + 12]
            if randnums[j] < Pn:
                dvn_count = dvn_count + 1
                stest[j] = nval.perturb_type(sbest[j], stest_lower, stest_upper, DV['Discrete_flag'][j])
        ...
        if dvn_count == 0:
            ...
            stest[dec_var] = nval.perturb_type(sbest[dec_var], DV['S_min'][dec_var], DV['S_max'][dec_var],
                                               DV['Discrete_flag'][dec_var])
