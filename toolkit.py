...
def read_param_file(filename):
    ...
    bounds_dat = np.loadtxt(filename, dtype={'names': ('S_name', 'S_min', 'S_max', 'Discrete_flag'),
                                             'formats': ('S3', 'f8', 'f8', 'i4')}, skiprows=1)
    return bounds_dat
...
def read_DDS_inp(filename):
    ...
    return {'objfunc_name': A[0], 'runname': A[1], 'num_trials': int(A[2]), 'num_iters': int(A[3]),
            'user_seed': int(A[4]), \
            'out_print': int(A[5]), 'ini_name': A[6], 'modeldir': A[7], 'obj_flag': int(A[8]), 'num_slaves': int(A[9]),
            'pre_empt_flag': int(A[10])}
...
def get_objfunc(x, modeldir, objfunc_name, exe_name, slave_index,
    ...
    if script_dir == modeldir:
        ...
        y = feval(x, Total_Copacity, lower_Capacity, Initial_Stor, Inflow_TS, Target_12TS, Ration_Ratio_48TS)
    else:
...
class solution:
    def __init__(self, decnum, objnum):
        self.dv = np.zeros(decnum, float)
        self.f = np.zeros(objnum, float)
        self.z = 0
